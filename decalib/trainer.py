    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        if self.cfg.loss.shape_consistency or self.cfg.loss.detail_consistency:
            ...
            if self.train_detail:
                ...
                # print(detailcode.shape,detailcode_new.shape)
                # [6,128] [6,128]-> [12,128]
        ...
        ###--------------- training detail model
            ...
            # mask
            mask_face_eye = F.grid_sample(self.deca.uv_face_eye_mask.expand(batch_size, -1, -1, -1),
                                          ops['grid'].detach(), align_corners=False)
            # images
            ...
            # 12,181
            d_input = torch.cat([posecode[:, 3:], expcode, detailcode], dim=1)
            # print(d_input.shape)
            uv_z = self.deca.D_detail(d_input)
            # print(uv_z.shape) -> 12,1,256,256
            # render detail
            ...
            losses['photo_detail'] = (
                                                 uv_texture_patch * uv_vis_mask_patch - uv_texture_gt_patch * uv_vis_mask_patch).abs().mean() * self.cfg.loss.photo_D
                ...
                if self.global_step % self.cfg.train.vis_steps == 0:
                    ...
                    visdict = {
                        ...
                        'landmarks2d_gt': util.tensor_vis_landmarks(opdict['images'][visind], opdict['lmk'][visind],
                                                                    isScale=True),
                        'landmarks2d': util.tensor_vis_landmarks(opdict['images'][visind],
                                                                 opdict['landmarks2d'][visind], isScale=True),
                    ...
                    grid_image = util.visualize_grid(visdict, savepath, return_gird=True)
                    # import ipdb; ipdb.set_trace()                    
                    self.writer.add_image('train_images', (grid_image / 255.).astype(np.float32).transpose(2, 0, 1),
                                          self.global_step)
